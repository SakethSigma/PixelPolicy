...
def main(argv: Optional[list[str]] = None) -> None:
    ...
    p.add_argument("--demo", action="store_true", help="watch one game on a colored board")
    p.add_argument("--episodes", type=int, default=10, help="headless: how many games")
    p.add_argument("--mode", choices=["train", "val"], default="val")
    p.add_argument("--word", default=None, help="pin the target word")
    p.add_argument("--concurrency", type=int, default=8, help="headless: parallel games")
    p.add_argument("--pace", type=float, default=0.0, help="demo: seconds between moves")
    p.add_argument("--step", action="store_true", help="demo: wait for Enter per move")
    ...
    from agents.backend import OpenAICompatBackend  # local: only running needs openai
    ...
    if args.demo:
        ...
        env = make_env()
        observer = TerminalObserver(render_board, pace=args.pace, step=args.step)
    ...
    print(f"win rate: {win_rate(trajs):.1%}  ({len(trajs)} games, mode={args.mode})")
